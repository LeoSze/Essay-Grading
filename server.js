...
app.post('/extract-text', upload.array('files', config.upload.maxFiles), async (req, res) => {
  try {
    ...
    // Get selected API key from request body (default to 0)
    const apiKeyNumber = parseInt(req.body.apiKeyNumber) || 0;
    // Get selected model from request body (default to gemini-2.5-flash)
    const modelName = req.body.modelName || 'gemini-2.5-flash';

    const result = await extractTextFromFiles(req.files, apiKeyNumber, modelName);
...
app.post('/evaluate-text', async (req, res) => {
  try {
    const { text, command, apiKeyNumber } = req.body;

    if (!text || text.trim() === '') {
      return res.status(400).json({ error: '請提供要評分的文字內容' });
    }

    // Get selected API key (default to 0)
    const keyNumber = parseInt(apiKeyNumber) || 0;
    // Get selected model from request body (default to gemini-2.5-flash)
    //const modelName = req.body.modelName || 'gemini-2.5-flash';
    const modelName = 'gemini-2.5-flash';

    const result = await evaluateWithGemini(text, command, keyNumber, modelName);
